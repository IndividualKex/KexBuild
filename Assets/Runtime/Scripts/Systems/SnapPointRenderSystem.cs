...
namespace KexBuild {
    ...
    public partial class SnapPointRenderSystem : SystemBase {
        ...
        protected override void OnUpdate() {
            ...
            if (settings.DuplicationMaterial == null || _lineMesh == null) return;
            ...
            var primaryColor = new float4(1f, 1f, 1f, 1f);
            var secondaryColor = new float4(0.4f, 0.6f, 0.8f, 0.6f);
            ...
            foreach (var buildable in SystemAPI.Query<Buildable>()) {
                ...
                for (int i = 0; i < buf.Length; i++) {
                    ...
                    float4 color = priority == 1 ? primaryColor : secondaryColor;
                    float size = priority == 1 ? half * 2f : half;

                    AddCross(world, size, color);
                }
            }
            ...
            if (_matrices.Length > 0) {
                RenderSnapPoints(settings.DuplicationMaterial);
            }
        }
        ...
        private void RenderSnapPoints(Material material) {
            ...
            var args = new uint[5] { _lineMesh.GetIndexCount(0), (uint)_matrices.Length, 0, 0, 0 };
            _argsBuffer.SetData(args);

            Graphics.RenderMeshIndirect(renderParams, _lineMesh, _argsBuffer);
        }
        ...
        private void CreateLineMesh() {
            ...
            _lineMesh.bounds = new Bounds(Vector3.zero, Vector3.one * 100f);
        }

        private void AddLine(float3 start, float3 end, float4 color) {
            ...
            quaternion rotation = quaternion.LookRotationSafe(normalizedDir, new float3(0, 1, 0));
            var matrix = float4x4.TRS(center, rotation, new float3(length, 1, 1));

            _matrices.Add(matrix);
            _visualizationData.Add(color);
        }

        private void AddCross(float3 position, float size, float4 color) {
            // X axis line
            AddLine(position - new float3(size, 0, 0), position + new float3(size, 0, 0), color);
            // Y axis line
            AddLine(position - new float3(0, size, 0), position + new float3(0, size, 0), color);
            // Z axis line
            AddLine(position - new float3(0, 0, size), position + new float3(0, 0, size), color);
        }
